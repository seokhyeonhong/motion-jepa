...
def make_classifier(
    ...
    *,
    ...
    if model_name == "cnn":
        ...
        model = MotionTransformerClassifier(**{key: value for key, value in config.items() if key != "name"})
    else:
...
def run_model(
    ...
    *,
    ...
    model_config["input_source"] = prepared.input_source
    if prepared.jepa_source is not None:
        model_config["jepa_source"] = prepared.jepa_source
    ...
    if args.overwrite:
        for name in existing + ["summary.json"]:
            path = output / name
            if path.exists():
                path.unlink()
    ...
    summary = {
        ...
        "stats_root": str(prepared.stats_root),
...
def write_findings(
    ...
    *,
    ...
    lines = [
        "# 100STYLE CNN and Transformer Classifiers",
        "",
...
def run(args: argparse.Namespace) -> dict[str, dict[str, Any]]:
    ...
    if input_source == "jepa":
        ...
        default_findings = PROJECT_ROOT / "findings/001-supervised-100style-classifiers"
    output_root = (
